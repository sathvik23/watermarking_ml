...
watermarkedFolder = 'dataset/watermarked';
watermarkFolder = 'dataset/watermarks';

% Create directories for saving watermarked images and watermark sequences
if ~exist(watermarkedFolder, 'dir')
    mkdir(watermarkedFolder);
end
if ~exist(watermarkFolder, 'dir')
    mkdir(watermarkFolder);
end

% Get list of original images
imageFiles = dir(fullfile(originalFolder, '*.png')); % Change extension if necessary
num_images = length(imageFiles);

% Parameters
imageSize = [512, 512];

% Generate watermarked images and save them along with watermark sequences
for i = 300:num_images
    % Read original image
    original_image_path = fullfile(originalFolder, imageFiles(i).name);
    % Check if the file is a valid image
    try
        original_image = imread(original_image_path);
    catch ME
        warning('Unable to read file "%s". Skipping file.', imageFiles(i).name);
        continue;
    end
    
    original_image = imresize(original_image, imageSize);
    % Define the length of the sequence
sequenceLength = 512;

% Create the sequence
bitSequence = [ones(1, sequenceLength/2), zeros(1, sequenceLength/2)];
    
    % Generate random watermark sequence
    robust_watermark = bitSequence;
    
    % Insert watermark into the original image
    watermarked_image = watermark_insertion(original_image, robust_watermark);
    
    % Save the watermarked image
    watermarkedImagePath = fullfile(watermarkedFolder, sprintf('image_%05d.png', i));
    imwrite(watermarked_image, watermarkedImagePath);
    
    % Save the watermark sequence
    watermarkPath = fullfile(watermarkFolder, sprintf('watermark_%05d.mat', i));
    save(watermarkPath, 'robust_watermark');
end

fprintf('Data preparation complete. Watermarked images and watermarks saved.\n');

% Functions for watermark insertion (included for completeness)
