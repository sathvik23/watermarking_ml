...
robust_watermark = randi([0, 1], 1, 1024);
%disp(robust_watermark)

% Secret key for block selection
secret_key = 4536;

% Function to insert a watermark
watermarked_image = watermark_insertion(original_image, robust_watermark, secret_key);
...
imwrite(watermarked_image, 'image_0036_watermarked.png');
imwrite(original_image,'image_0036.png')
fprintf('Watermarked image saved as watermarked_image.png\n');
...
function watermarked_image = watermark_insertion(original_image, robust_watermark, secret_key)
    block_size = 16;
    ...
    rng(secret_key); % Initialize random seed
    disp(rng(secret_key))
    block_indices = randperm(num_blocks, length(robust_watermark)); % Random block selection
    disp("BLOCK INDICES: ");
    disp(block_indices)
...
function padded_array = pad_array(array, target_length)  % we need lengths to match for concat
    % Pads the array with zeros to reach the target length
    if size(array, 1) == 1
        padded_array = [array'; zeros(target_length - length(array), 1)];
    else
        padded_array = [array; zeros(target_length - length(array), 1)];
    end
end
